...
export class Mygeteway implements OnGatewayInit, OnGatewayConnection {
  ...
  handleDisconnect(client: Socket) {
    ...
    for (let [key, value] of this.roomData) {
      for (let i = 0; i < value.length; i++) {
        if (
          value[i].player1?.socketId == socketId ||
          value[i].player2?.socketId == socketId
        ) {
  ...
  reqToJoin(@ConnectedSocket() socket: Socket) {
    ...
    for (let [key, value] of this.roomData) {
      for (let i = 0; i < value.length; i++) {
        if (
          value[i].player?.socketId == socket.id ||
          value[i].player?.socketId == socket.id
        ) {
          exist = 1;
          break;
  ...
  startGame(@MessageBody() data: any) {
    ...
    const interval = setInterval(() => {
      ...
      if (
        ...
        if (ballPos.y > 0) this.roomData.get(roomName)[1].player.score++;
        else if (ballPos.y < 0) this.roomData.get(roomName)[2].player.score++;
        this.resetBall(roomName);
        this.resetPlayers(roomName);
        if (
          this.roomData.get(roomName)[1].player.score == 10 ||
          this.roomData.get(roomName)[2].player.score == 10
        ) {
  ...
  player1(@MessageBody() data: any) {
    ...
    if (socketId == room[1].player.socketId) {
      ...
      if (right && player.x + 3 < w) player.x += 3;
      else if (left && player.x - 3 > -w) player.x -= 3;
