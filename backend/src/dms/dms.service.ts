...
export class DmsServices {
  ...
  async getAllDmsOfUser(userId: number) {
    const conversationIds = await this.prisma.user_Conv
      .findMany({
        where: {
    ...
    const dms = await this.prisma.user_Conv
      .findMany({
        where: {
          user: {
            isNot: {
      ...
      .then((dms) =>
        dms.map(({ conversationId, user }) => ({ conversationId, ...user })),
      );
    return dms;
  }
  ...
  async createDm(data) {
    ...
    const createDm = await this.prisma.conversation.create({
      data: conversation,
    });
    if (createDm) {
